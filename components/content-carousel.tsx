      ...
      >
        {content.map((item) => {
          ...
          return (
            <Card
              key={item.id}
              className={`bg-transparent border-none overflow-hidden group cursor-pointer flex-shrink-0 ${
                compact ? (isMobile ? "w-48" : "w-60") : isMobile ? "w-64" : "w-80"
              }`}
              onClick={() => {
            ...
            >
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-colors duration-300 rounded-lg flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Button
                      size="icon"
                      className={`bg-white/20 hover:bg-white/30 text-white border-none rounded-full ${
                ...
                {/* Duration Badge */}
                <div
                  className={`absolute bottom-2 right-2 bg-black/80 text-white rounded ${
                ...
                {item.episodeNumber && (
                  <div
                    className={`absolute top-2 left-2 bg-red-600 text-white font-bold rounded ${
                      isMobile ? "text-xs px-1.5 py-0.5" : "text-xs px-2 py-1"
                    }`}
                  >
              ...
              <div className={`${compact ? (isMobile ? "p-1.5" : "p-2") : isMobile ? "p-2" : "p-3"}`}>
                <h3
                  className={`text-white font-semibold group-hover:text-blue-400 transition-colors line-clamp-2 mb-1 md:mb-2 ${
                    compact
                      ? isMobile
                        ? "text-xs"
